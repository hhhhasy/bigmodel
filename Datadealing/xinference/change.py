            ...
            for line in file:
                ...
                try:
                    ...
                    for model in model_list:
                        ...
                        model_version = model.get('model_version', '').strip()

                        full_model_name = f"{model_name}:{model_version}" if model_version else model_name
                        base_model_name = model_name.split('/')[-1].split(':')[0]
...
def main():
    ...
    for filename in os.listdir(model_folder):
        match = re.match(r'server_xinference_(\d{8})_ip_9997_alive_models\.txt', filename)
        if match:
            date_str = match.group(1)
            input_model_path = os.path.join(model_folder, filename)
            output_model_csv = os.path.join(csv_folder, f'xinference_extracted_models_{date_str}.csv')
    ...
    for filename in os.listdir(ip_folder):
        match = re.match(r'server_xinference_(\d{8})_ip_9997_alive\.txt', filename)
        if match:
            date_str = match.group(1)
            input_ip_path = os.path.join(ip_folder, filename)
            output_ip_csv = os.path.join(csv_folder, f'xinference_{date_str}_ip_location.csv')
