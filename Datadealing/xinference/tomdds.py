...
# 遍历所有 xinference_extracted_models_YYYYMMDD.csv 文件
for filename in os.listdir(csv_dir):
    match = re.match(r'xinference_extracted_models_(\d{8})\.csv', filename)
    if match:
        date_str = match.group(1)
        input_file_path = os.path.join(csv_dir, filename)

        # 输出文件路径
        base_model_output = os.path.join(csv_dir, f'xinference_{date_str}_count_md.csv')
        full_model_output = os.path.join(csv_dir, f'xinference_{date_str}_count_mdver.csv')
        deepseek_output    = os.path.join(csv_dir, f'xinference_{date_str}_count_deepseek.csv')
        asn_count_output = os.path.join(csv_dir, f'xinference_{date_str}_count_asn.csv')
        asn_org_output = os.path.join(csv_dir, f'xinference_{date_str}_count_asn_org.csv')

        # 如果文件都存在，就跳过
        if all(os.path.exists(f) for f in [base_model_output, full_model_output, deepseek_output, asn_count_output, asn_org_output]):
    ...
    match = re.match(r'xinference_(\d{8})_ip_location\.csv', filename)
    if match:
        date_str = match.group(1)
        input_file_path = os.path.join(csv_dir, filename)
