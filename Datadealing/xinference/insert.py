...
def create_daily_expose_table(cursor):
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS xinference_daily_expose (
...
def insert_into_daily_expose(cursor, date_str, ip_set):
    ...
    cursor.execute("SELECT * FROM xinference_daily_expose WHERE Date = %s", (date_str,))
    if cursor.fetchone():
        print(f"[✓] xinference_daily_expose 已存在记录：{date_str}，跳过")
        return
    cursor.execute("INSERT INTO xinference_daily_expose (Date, counts) VALUES (%s, %s)", (date_str, len(ip_set)))
    print(f"[+] 插入到 xinference_daily_expose：{date_str} → {len(ip_set)} IPs")
...
def main():
    try:
        ...
        for filename in os.listdir(CSV_FOLDER):
            ...
            # match = re.match(r'xinference_(\d{8})_(ip_location|count_md|count_deepseek)\.csv', filename)
            match = re.match(r'xinference_(\d{8})_(ip_location|count_md)\.csv', filename)
            if not match:
                continue

            date_str_raw, file_type = match.groups()
            table_name = f'xinference_{date_str_raw}_{file_type}'
