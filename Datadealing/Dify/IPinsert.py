...
def create_daily_expose_table(cursor):
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS Dify_daily_expose (
...
def insert_into_daily_expose(cursor, date_str, ip_set):
    ...
    cursor.execute("SELECT * FROM Dify_daily_expose WHERE Date = %s", (date_str,))
    if cursor.fetchone():
        print(f"[✓] Dify_daily_expose 已存在记录：{date_str}，跳过")
        return
    cursor.execute("INSERT INTO Dify_daily_expose (Date, counts) VALUES (%s, %s)", (date_str, len(ip_set)))
    print(f"[+] 插入到 Dify_daily_expose：{date_str} → {len(ip_set)} IPs")
...
def main():
    try:
        ...
        for filename in os.listdir(CSV_FOLDER):
            ...
            match = re.match(r'Dify_(\d{8})_(ip_location|count_asn|count_asn_org)\.csv', filename)
            if not match:
                continue

            date_str_raw, file_type = match.groups()
            table_name = f'Dify_{date_str_raw}_{file_type}'
