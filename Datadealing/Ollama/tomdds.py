...
for filename in os.listdir(csv_dir):
    ...
    if match:
        ...
        asn_count_output = os.path.join(csv_dir, f'ollama_{date_str}_count_asn.csv')
        asn_org_output = os.path.join(csv_dir, f'ollama_{date_str}_count_asn_org.csv')
    ...
    match = re.match(r'ollama_(\d{8})_ip_location\.csv', filename)
    if match:
        date_str = match.group(1)
        input_file_path = os.path.join(csv_dir, filename)
