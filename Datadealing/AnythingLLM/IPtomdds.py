...
# 遍历所有 AnythingLLM_YYYYMMDD_ip_location.csv 文件
for filename in os.listdir(csv_dir):
    match = re.match(r'AnythingLLM_(\d{8})_ip_location\.csv', filename)
    if match:
        date_str = match.group(1)
        input_file_path = os.path.join(csv_dir, filename)

        # 输出文件路径
        asn_count_output = os.path.join(csv_dir, f'AnythingLLM_{date_str}_count_asn.csv')
        asn_org_output = os.path.join(csv_dir, f'AnythingLLM_{date_str}_count_asn_org.csv')
