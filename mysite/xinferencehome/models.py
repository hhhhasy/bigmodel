...
def get_latest_count_asn():
    ...
    with connection.cursor() as cursor:
        cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'bigmodel'")
        tables = [row[0] for row in cursor.fetchall()]
    pattern = re.compile(r'^xinference_(\d{8})_count_asn$')
    matched = [(m.group(1), tbl) for tbl in tables if (m := pattern.match(tbl))]
    if not matched:
        raise Exception("找不到符合格式的 xinference_{date}_count_asn 表。")
    latest_date, latest_table = max(matched, key=lambda x: x[0])
    return latest_table
...
def get_latest_count_asn_org():
    ...
    with connection.cursor() as cursor:
        cursor.execute("SELECT table_name FROM information_schema.tables WHERE table_schema = 'bigmodel'")
        tables = [row[0] for row in cursor.fetchall()]
    pattern = re.compile(r'^xinference_(\d{8})_count_asn_org$')
    matched = [(m.group(1), tbl) for tbl in tables if (m := pattern.match(tbl))]
    if not matched:
        raise Exception("找不到符合格式的 xinference_{date}_count_count_asn 表。")
    latest_date, latest_table = max(matched, key=lambda x: x[0])
    return latest_table
