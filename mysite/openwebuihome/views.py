...
from .models import DailyExpose, OpenWebUIHoneypot
...
def dashboard(request):
    ...
    # 获取蜜罐数据的最新日期
    latest_honeypot = OpenWebUIHoneypot.objects.aggregate(max_timestamp=Max('timestamp'))
    ...
    if latest_date:
        ...
        start_date = (end_date - timedelta(days=6)).replace(hour=0, minute=0, second=0, microsecond=0)
    # 获取最近7天的蜜罐数据
    honeypot_attacks_qs = OpenWebUIHoneypot.objects.filter(
    ...
    # 统计 Top N 攻击数据
    top_ports = OpenWebUIHoneypot.objects.filter(
    ...
    top_locations = OpenWebUIHoneypot.objects.filter(
        timestamp__range=(start_date, end_date)
    ...
    top_domains = OpenWebUIHoneypot.objects.filter(
    ...
    return render(request, 'openwebuihome/home.html', {
