...
from .models import DailyExpose, OpenLLMHoneypot
...
def dashboard(request):
    ...
    # 获取蜜罐数据的最新日期
    latest_honeypot = OpenLLMHoneypot.objects.aggregate(max_timestamp=Max('timestamp'))
    ...
    # 获取最近7天的蜜罐数据
    honeypot_attacks_qs = OpenLLMHoneypot.objects.filter(
    ...
    top_locations = OpenLLMHoneypot.objects.filter(
        timestamp__range=(start_date, end_date)
    ...
    top_domains = OpenLLMHoneypot.objects.filter(
